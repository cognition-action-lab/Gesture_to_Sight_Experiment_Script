...
HandCursor::HandCursor(GLfloat x, GLfloat y, GLfloat z, GLfloat diam, GLfloat clr[]) : Circle(x, y, diam, clr)
{
...
void HandCursor::UpdatePos(GLfloat x, GLfloat y, GLfloat z)
{

	GLfloat xTrue, yTrue, zTrue;
	...
	zvel = (-2*zhist[4] -1*zhist[3] + 1*zhist[1] + 2*zhist[0])*SAMPRATE/10;
...
GLfloat HandCursor::GetMeanX()
{
	float meanx = 0;
	
	for (int a = 0; a < NCHIST; a++)
		meanx += xhist[a];
	meanx = meanx/(float(NCHIST));

	return (meanx);  //xhist[0] is the current sample
}

GLfloat HandCursor::GetMeanY()
{
	float meany = 0;
	
	for (int a = 0; a < NCHIST; a++)
		meany += yhist[a];
	meany = meany/(float(NCHIST));

	return (meany);  //xhist[0] is the current sample
}

GLfloat HandCursor::GetMeanZ()
{
	float meanz = 0;
	
	for (int a = 0; a < NCHIST; a++)
		meanz += yhist[a];
	meanz = meanz/(float(NCHIST));

	return (meanz);  //xhist[0] is the current sample
}
...
GLfloat HandCursor::GetVel2D()
{
	return sqrtf(xvel*xvel + yvel*yvel);   //return the true hand velocity (non-distorted) in 2D
}

GLfloat HandCursor::GetVel3D()
{
	return sqrtf(xvel*xvel + yvel*yvel + zvel*zvel);   //return the true hand velocity (non-distorted) in 3D
}


GLfloat HandCursor::Distance(GLfloat x, GLfloat y, GLfloat z)
{
	return sqrtf(powf(xpos - x, 2.0f) + powf(ypos - y, 2.0f) + powf(zpos - z, 2.0f));
}

GLfloat HandCursor::Distance(Circle* c)
{
	//return sqrtf(powf(xpos - c->GetX(), 2.0f) + powf(ypos - c->GetY(),2.0f));
	return sqrtf(powf(xpos - c->GetX(), 2.0f) + powf(ypos - c->GetY(),2.0f) + powf(zpos - c->GetZ(),2.0f));
}
