...
struct TrackSYSCONFIG
{
	//general parameters
	double trackType;
	SYSTEM_CONFIGURATION systemconfig;  //for trakSTAR, system configuration structure
	TRANSMITTER_CONFIGURATION Xmtr[10];  //for trakSTAR, array of transmitter configuration structures. assume we have less than 10 transmitters (boards) possible!
	USHORT xmtrloc;  //index into the transmitter array pointing to where the transmitter is
	...
	//offsets
	double PosOffset[3]; //x,y,z offsets
	double RotOffset[2]; //azimuth,elevation offsets
