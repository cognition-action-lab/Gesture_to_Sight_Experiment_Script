...
public:
	...
	Circle(GLfloat x, GLfloat y, GLfloat z, GLfloat diam, GLfloat clr[]);
	...
	void SetPos(GLfloat x, GLfloat y, GLfloat z = 0.0f);
	void SetVel(GLfloat vx, GLfloat vy);	//set a fixed x,y velocity of the circle (for pursuit targets)

	// draw the circle
	void Draw();
	void UpdatePosition();  //update circle position according to velocity and timer info
